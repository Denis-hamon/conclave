...
import json
from conclave.router import TaskRouter, ModelTier, ExecutorType
...
def test_routes_repetitive_task_to_haiku(mock_client):
    _set_classifier(mock_client, {
        "novelty": 0.1, "complexity": 0.2,
        "is_repetitive": True, "needs_filesystem": False,
...
def test_routes_filesystem_task_to_deepagents(mock_client):
    _set_classifier(mock_client, {
        "novelty": 0.5, "complexity": 0.5,
        "is_repetitive": False, "needs_filesystem": True,
...
def test_force_model_override(mock_client):
    ...
    mock_client.messages.create.side_effect = [
        mock_client._make_response(json.dumps({
            "novelty": 0.9, "complexity": 0.9,
            "is_repetitive": False, "needs_filesystem": False,
            "rationale": "complex",
        })),
    ...
    agent = ConclaveAgent(
        role="Lead", persona="lead", org_name="T", tools=[],
        reports_to=None, org_structure="Lead", deliberation="hierarchy",
        client=mock_client, force_model="claude-haiku-4-5-20251001",
    )
